                    ...
                    for alpha in 0.7 0.75 0.8 0.85 0.9 0.95; do
                        for lr in 0.001 0.005 0.01 0.05; do
                            for prob in 0.7 0.75 0.8 0.85 0.9; do
                                ...
                                echo $output
                                bsub -W 24:00 -n 1 -R "rusage[mem=4096]" -J "$game" -oo $OUTPUT_DIR/$output python environment.py --game $game --estimator $estimator --output $OUTPUT_DIR --iter $ITERATIONS --num_traj $TRAJECTORIES --prob $probability --alpha $alpha --batch_size $batch_size --mini_batch_size $mini_batch_size --lr $lr
