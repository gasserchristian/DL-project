...
class lunar_lander(game):
    def __init__(self):
        ...
        # policy "snapshot" network used by some algorithms
    ...
    # performs the evaluation of the current policy NN for
    def evaluate(self, number_of_runs=10):
        # a given number of runs
        number_of_sampled_trajectories = self.number_of_sampled_trajectories
        results = [np.sum(self.sample(200)['rewards'])
    ...
    def generate_data(self, estimator, number_of_sampled_trajectories=10000):
        ...
        while True:
            # performs one step of update for the selected estimator
            estimator.step(self)
            ...
            if self.number_of_sampled_trajectories > number_of_sampled_trajectories:
                ...
                break
        np.savetxt('data--'+type(self).__name__+'_' +
                   type(estimator).__name__+'.txt', np.array(evaluations).transpose())
