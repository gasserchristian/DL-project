...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            action = self.pi.forward(state)[0].sample() 
            value_function = self.v.forward(state) 
            log_prob_action = self.pi.forward(state, action)[1]

        # convert tensors to numerical values 
        action = action.detach().numpy()  
        value_function = value_function.detach().numpy()
        log_prob_action = log_prob_action.detach().numpy()

        return action, value_function, log_prob_action
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        deltas = r_t + self.gamma*V_t_1 - V_t # TD residual of V with discount gamma 
        self.phi_buf[path_slice] = discount_cumsum(deltas, self.gamma*self.lam) # formula 16 from GAE paper 
...
class Agent:
    ...
    def pi_update(self, data):
        ...
        ret = data['ret'] # remaining return over the trajectory; is it discounted or not? 
        
        log_p = self.ac.pi.forward(obs,act)[1]
        loss = -log_p*phi 
        loss = loss.sum()
    ...
    def v_update(self, data):
        ...
        obs = data['obs'] # 5 dimensional vectors
        act = data['act'] # actions  
        phi = data['phi'] # advantage estimates???
        ret = data['ret'] # remaining return along the trajectory 
        ...
        for _ in range(n_updates):
            ...
            target = ret # our target is the remaining return                 
            v_nn = self.ac.v.forward(obs) # we optimize this approximation of the value function
            mean_squared_error = nn.MSELoss()
            loss = mean_squared_error(v_nn, target)
            loss.backward()
    ...
    def train(self):
        ...
        steps_per_epoch = 3000 # what is one step? 
    ...
    def get_action(self, obs):
        ...
        action = self.ac.step(torch.Tensor(obs))[0] 
        return action
