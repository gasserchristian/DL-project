...
class svrpg_manual(estimator):
	...
	def step(self, game):
		...
		if (self.adam_lr_snapshot / self.N) > (self.adam_lr_current / self.B): 
			"""
	...
	def network_update(self, v, game, first_iteration = False): # update all weights of current network 
		...
		# adam estimates for increments and for the learning rate: 
		if first_iteration == False: # for remaining subiterations 
			adam_increments, self.adam_lr_current = self.current_adam.increment_and_lr(v) 
		else: # for first subiteration 
			adam_increments, self.adam_lr_snapshot = self.snapshot_adam.increment_and_lr(v)

		# this updates current network with adam increments 
		for (policy_name, policy_values), (adam_name, adam_values) in zip(policy_dict.items(), adam_increments.items()):
			policy_dict[policy_name].copy_(policy_values + adam_values) # updates current network 
